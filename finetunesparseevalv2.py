...
class SparseAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        sparse_indices = torch.nonzero(mask.unsqueeze(1), as_tuple=True)
        sparse_values = attn_scores[sparse_indices].view(-1)
        sparse_size = list(attn_scores.size())
        sparse_size[-2] = 1  # Only one element per row for sparse tensor
        attn_scores_sparse = torch.sparse.FloatTensor(sparse_indices, sparse_values, sparse_size)

        # Compute attention probabilities
        attn_probs_sparse = F.softmax(attn_scores_sparse.to_dense(), dim=-1)
        attn_probs_sparse = self.dropout_layer(attn_probs_sparse)

        # Apply attention to values
        attn_output_sparse = torch.matmul(attn_probs_sparse, value)
